...
const CoachProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setProfile(res.data);
      } catch (err) { toast.error("Profile not loaded"); console.log(err); }
    };
    fetchProfile();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error("Update failed"); console.log(err);
    } finally { 
      setLoading(false); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
      ...
      <div className="max-w-2xl mx-auto p-6">
        <div className="rounded-2xl overflow-hidden border border-green-500/20 mb-6">
          ...
          <div className="bg-gradient-to-r from-gray-800 to-gray-700 px-6 py-8 flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">{profile.name || "Coach"}</h2>
              <p className="text-green-200 text-sm mt-1">Coach</p>
            </div>
          </div>
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 rounded-xl text-sm outline-none transition-all
                      ${editing && !disabled
                        ? "bg-green-900/60 border border-green-500/50 text-white"
                        : "bg-white/5 border border-white/10 text-green-100 cursor-default"
                      }`}
                  />
                </div>
              ))}
              ...
              {editing && (
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    {avatarSrc && <img src={avatarSrc} alt="preview" className="w-14 h-14 rounded-full object-cover border-2 border-green-500/40" />}
                    <button onClick={() => fileRef.current.click()}
                      className="flex items-center gap-2 px-4 py-2 rounded-xl border border-green-500/40 text-green-300 text-sm hover:bg-green-500/15 transition-all cursor-pointer">
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={handleSave} disabled={loading}
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all cursor-pointer
                      ${loading ? "bg-green-700/50 cursor-not-allowed text-green-300" : "bg-gradient-to-r from-green-600 to-green-700 hover:opacity-90 text-white"}`}>
